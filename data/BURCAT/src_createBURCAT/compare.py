...
#==========================================================================
# read UMIST Heat_of_formation.dat
#==========================================================================
print
print "reading Heat_of_formation.dat ..."
f = open('Heat_of_formation.dat','r')
dum = f.readline()
Ndat = int(f.readline())
lines = f.readlines()
f.close
UMnam = []
UMdH0 = []  # standard heat of formation at 0K [kJ/mol]
for i in range(Ndat):
  line = lines[i]
  #print line
  dat = line.split()[1]
  if ('..' not in dat):
    UMnam.append(line.split()[0].upper())
    UMdH0.append(float(dat))
UMnam = np.array(UMnam)
UMdH0 = np.array(UMdH0)
...
for mol in allnam:
  ...
  plt.xlim(-70,1.02*Tmax)
  #plt.xlabel(r'$T\,\mathrm{[K]}$',fontsize=16)
  plt.ylabel(r'$\Delta_\mathrm{f} G^0\ \mathrm{[kJ/mol]}$',fontsize=16)
  plt.title(mol)

  ind = np.where(UMnam==mol)[0]
  if (len(ind)>0):
    i = ind[0]
    dHf0 = UMdH0[i]
    print "->",i,UMnam[i]
    plt.scatter(0.0,dHf0,s=150,c='black',marker='*',label='UMIST',zorder=10)
